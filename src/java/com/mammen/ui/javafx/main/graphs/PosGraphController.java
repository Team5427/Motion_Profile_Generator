...
public class PosGraphController
{
    ...
    private void setBGImg()
    {
        ...
        {
            File imgFile = new File( SettingsModel.getSettingsDir() + "/DeepSpace.jpg" );

            if( !imgFile.exists() )
            {
                try
                {
                    ResourceLoader.resourceToFile("/images/DeepSpace.jpg", imgFile );
                }
                catch( IOException e )
                {
                    return;
            ...
            settings.setGraphBGImagePath( SettingsModel.getSettingsDir() + "/DeepSpace.jpg" );
        }
        else
        {
